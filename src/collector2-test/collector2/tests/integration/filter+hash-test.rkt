...
(module+ test
  ...
  (define test-hash
    (hash "custom0"
          (hash 'name "custom0" 'source "https://gitlab.com/asd/custom0.git")
          "custom1"
          (hash 'name "custom1" 'tags '("main-distribution"))
          "custom2"
          (hash 'name
                "custom2"
                'dependencies
                ...
                '("main-distribution"))
          "custom3"
          (hash 'name "custom3" 'dependencies '("custom2"))
          "custom4"
          (hash 'name "custom4" 'dependencies '("custom3"))
          "custom5"
          (hash 'name "custom5" 'dependencies '("custom0" "custom5" "none"))))
  ...
  (check-eq?
   (numkeys (hash-filter (lambda (h)
                           (equal? (car (hash-ref h 'tags '("")))
                                   "main-distribution"))
                         test-hash))
   2)

  (check-equal?
   (length (hash-ref (hash-key-set-subtract
                      (hash-ref test-hash "custom5")
                      'dependencies
                      '("custom0"))
                     'dependencies))
   2)
  ...
  (check-eq?
   (length (hash-ref (hash-ref (hash-remove-missing-dependencies
                                test-hash)
                               "custom5")
                     'dependencies))
   2)
